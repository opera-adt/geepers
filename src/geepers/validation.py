...
from geepers.analysis import (  # noqa: F401  (re-exports)
    binned_rmse_profile,
    epoch_rmse,
...
def _fit_one(
    ...
    t = (dates[good] - dates[good][0]).total_seconds().to_numpy() / 86400 / DAYS_PER_YEAR
    v = values[good]
...
def comparison_stats(x: np.ndarray, y: np.ndarray) -> dict[str, float]:
    ...
    x, y = x[good], y[good]
    if len(x) < 2:
        return {"bias": np.nan, "mad": np.nan, "rmse": np.nan, "r2": np.nan,
                "n": int(len(x))}
    d = y - x
...
def plot_velocity_scatter(
    ...
    ax.errorbar(
        ok["insar_velocity"], ok["gps_velocity"], xerr=xerr, yerr=yerr,
        fmt="o", ms=6, capsize=2, zorder=3,
    )
    if annotate:
        for sid, row in ok.iterrows():
            ax.annotate(str(sid), (row["insar_velocity"], row["gps_velocity"]),
...
def plot_semivariogram(
    ...
    ax.scatter(rmse_df["distance_km"], gamma, s=25, c="0.2", alpha=0.6,
               zorder=3, label="station pairs")
    if binned is not None and len(binned):
        ax.plot(binned["distance_km"], binned["gamma_median"], "r-o",
                ms=4, lw=1.5, zorder=4, label="median per bin")
    ...
    ax.set_ylabel(f"Semivariance γ ({units}²)")
