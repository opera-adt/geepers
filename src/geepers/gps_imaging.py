    ...
    *,
    ...
    for i, iw in enumerate(neighborhoods):
        ...
        if robust_network:
            ...
            md = np.median(dist_all[iw])
            iw = np.union1d(iw, np.flatnonzero(dist_all <= md))
        dist = great_circle_degrees(lat2[i], lon2[i], lat2[iw], lon2[iw])
        out2[i], _ = _neighborhood_estimate(dist, v2[iw], s2[iw], ssf, method)
