r"""Euler pole estimation and plate-motion prediction from GNSS velocities.
...
def _design_matrix(lon: np.ndarray, lat: np.ndarray, height: np.ndarray) -> np.ndarray:
...
class EulerPole:
    ...
    def uncertainty(self) -> dict[str, float]:
        ...
        """
        R = _ecef_to_enu_matrices(np.atleast_1d(self.lon), np.atleast_1d(self.lat))[0]
