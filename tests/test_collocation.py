...
class TestCollocation:
    ...
    def test_interpolate_velocities_end_to_end(self, stations):
        ...
        at_sta, at_new = interpolate_velocities(
            lon, lat, ve, vn,
            np.full(n, 0.3), np.full(n, 0.3),
            lon_new, lat_new,
...
class TestOrdinaryKriging:
    ...
    def test_sigma_grows_away_from_data(self, stations):
        ...
        near = ordinary_kriging(
            lon, lat, ve, np.full(n, 0.1),
            [lon.mean()], [lat.mean()], np.array([4.0, 150.0]),
        )
        far = ordinary_kriging(
            lon, lat, ve, np.full(n, 0.1),
            [lon.mean() + 15], [lat.mean()], np.array([4.0, 150.0]),
        )
    ...
    def test_matches_collocation_on_demeaned_data(self, stations):
        ...
        from geepers.collocation import (
            collocate, noise_covariance, ordinary_kriging, predict,
            signal_covariance,
        )
        ...
        np.testing.assert_allclose(
            ok.signal.ravel(), lsc.signal.ravel(), atol=0.3
        )
...
class TestSeparatePlates:
    ...
    def test_plates_pushed_apart(self, two_plates):
        ...
        lon_m, lat_m, idx = separate_plates(
            lon, lat, plates, min_separation_km=1500
        )
        assert set(idx) == {0, 1}
        a, b = idx == 0, idx == 1
